...
class AccountSettingsActivity : AppCompatActivity()
{
    ...
    override fun onCreate(savedInstanceState: Bundle?)
    {
        ...
        //
        val deleteBtn: Button = findViewById<Button>(R.id.delete_account_btn)
        deleteBtn?.setOnClickListener(){
            val user = FirebaseAuth.getInstance().currentUser
            Toast.makeText(applicationContext,"Your Account Deleted Successfully",Toast.LENGTH_SHORT).show()
    ...
    private fun updateUserInfoOnly()
    {
        when {
            ...
            bio_profile_frag.text.toString() == "" -> Toast.makeText(this, "Please write your bio first.", Toast.LENGTH_LONG).show()
                ...
                userMap["propername"]=  full_name_profile_frag.text.toString().toLowerCase()
        ...
        usersRef.addValueEventListener(object : ValueEventListener
        {
            override fun onDataChange(p0: DataSnapshot)
            {
                if (p0.exists())
                {
                    ...
                    Picasso.get().load(p0.child("image").getValue().toString()).placeholder(R.drawable.profile).into(profile_image_view_profile_frag)
